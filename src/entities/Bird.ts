...
export class Bird {
  ...
  private jumpLift = -6.5664;   // Reduced by 5% (was -6.912)
  private maxFallSpeed = 11.88;  // +20% increase (was 9.9)
  private maxRiseSpeed = -8.721; // Reduced by 5% (was -9.18)
  ...
  public setDifficulty(difficulty: 'easy' | 'medium' | 'hard') {
    ...
    this.jumpLift = -6.5664;    // Reduced by 5% (was -6.912)
    this.maxFallSpeed = 11.88;   // +20% increase (was 9.9)
    this.maxRiseSpeed = -8.721;  // Reduced by 5% (was -9.18)
  }
  ...
  private getLevelVelocityReduction(engine: any): number {
    if (engine && engine.gameMode === 'level') {
      ...
      if (lvlNum === 1) return 0.90; // 10% reduction
      if (lvlNum === 2) return 0.93; // 7% reduction
      if (lvlNum >= 3 && lvlNum <= 10) return 0.88; // 12% reduction
    }
    return 1.0;
  }
  ...
  public jump(soundManager?: any, score = 0) {
    ...
    const jumpReduction = this.getLevelVelocityReduction(engine);
  ...
  public update(deltaTime: number, particleEngine: ParticleEngine, isPlaying: boolean, timeScale: number, score = 0) {
    ...
    const engine = (window as any).gameEngine;
    const speedReduction = this.getLevelVelocityReduction(engine);
    ...
    let currentMaxRiseSpeed = this.maxRiseSpeed * jumpScale * speedReduction;
    ...
    if (engine && engine.activePowerupsList['mini']) {
      ...
      this.sizeMultiplier = 0.60; // Ultimate Micro Size!
  ...
  private drawWhiteDragon(ctx: CanvasRenderingContext2D) {
    ...
    // Small sharp golden bird beak (falcon-like curved hook, scaled down to look smaller & elegant)
    const beakGrad = ctx.createLinearGradient(headX + 3, headY - 3, headX + 11, headY + 3);
    beakGrad.addColorStop(0, '#ffd54f'); // Golden yellow base
    beakGrad.addColorStop(0.6, '#ff8f00'); // Deep gold
    ...
    ctx.beginPath();
    ctx.moveTo(headX + 4.2, headY - 2.4); // top base of beak
    ctx.quadraticCurveTo(headX + 10.8, headY - 1.8, headX + 9.6, headY + 2.4); // curved hook tip pointing down
    ctx.lineTo(headX + 7.2, headY + 1.2); // lower cut
    ctx.quadraticCurveTo(headX + 5.5, headY + 0.3, headX + 3, headY + 0.5); // mouth line connection
    ctx.closePath();
    ctx.fill();
    ctx.stroke();

    // Subtle beak highlight reflection line
    ctx.strokeStyle = '#ffffff';
    ctx.lineWidth = 0.6;
    ctx.beginPath();
    ctx.moveTo(headX + 4.8, headY - 1.8);
